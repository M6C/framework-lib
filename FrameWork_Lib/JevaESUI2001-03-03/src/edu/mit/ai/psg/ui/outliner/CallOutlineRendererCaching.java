...
// Source File Name:   ui/outliner/CallOutlineRendererCaching.java
...
//            OutlineRendererCaching, CallOutlineNodeDefault, CallOutlineNode, OutlineNode, 
//            CallOutlineRenderer, OutlineModel, OutlineRendererDefault, OutlineMaker

public class CallOutlineRendererCaching extends OutlineRendererCaching
    implements CallOutlineRenderer
{

    public CallOutlineRendererCaching(OutlineMaker outlineMaker)
    {
        super(outlineMaker);
    }
    ...
    public void notifyRemoving(OutlineModel model, OutlineNode child)
    {
        ...
        if(firstNotified == null)
        {
            ...
            try
            {
                ...
                CallOutlineNode partner = callOutlineNode.getPartner();
                if(partner != null)
                    if(partner.getRenderer() instanceof CallOutlineRendererCaching)
                        model.removeNodeFromParent(partner);
                    else
                        throw new IllegalStateException("partner renderer " + partner.getRenderer() + " is not instanceof CallOutlineRendererCaching");
            }
            finally
            {
